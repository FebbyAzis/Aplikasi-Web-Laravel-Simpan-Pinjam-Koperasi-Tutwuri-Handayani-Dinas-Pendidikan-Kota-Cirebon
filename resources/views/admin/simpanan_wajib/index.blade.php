              ...
              <div class="card">
                 <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                       <h4 class="card-title">Tabel Data Simpanan Wajib</h4>
                    </div>
                 </div>
                 <div class="card-body">
                    <p>Berikut adalah tabel data simpanan wajib seluruh anggota koperasi.</p>
                    @if (session('Success'))
                    ...
                    <div class="table-responsive">
                       <table id="datatable" class="table table-striped" data-toggle="data-table">
                          ...
                          <tbody>
                            @foreach ($simw as $no=>$item)
                            <tr>
                                <td>{{ $no+1 }}</td>
                                <td><a href="{{route('simpanan-wajib.show', $item->id)}}">{{ $item->anggota->nama }}</a></td>
                            
                                <td>
                                    <form>
                                        {{-- <a class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop2{{$item->id}}">EDIT</a> --}}
                                        <a class="btn btn-sm btn-primary" href="{{route('simpanan-wajib.show', $item->id)}}">LIHAT</a>
                                    </form>
                                </td>
                             </tr>
                             @endforeach
           ...
           <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Lengkapi data dibawah ini untuk menambahkan simpanan wajib baru!</p>
                        <div class="row">
                            <div class="form-group">
                                <label class="form-label" for="choices-single-default">Pilih Anggota</label>
                                <select class="form-select" data-trigger name="anggota_id" id="users_id">
                                    <option value="0">-</option>
                                    @foreach ($angg as $s)
                                    @if (!in_array($s->id, $simwOptions))
...
<script type="text/javascript">
    const selectUser = document.getElementById('anggota_id');
    const totalDataInput = document.getElementById('totalData');

    selectUser.addEventListener('change', function() {
        const selectedUserId = this.value;
        if (selectedUserId) {
            fetch(`/calculate/${selectedUserId}`)
                .then(response => response.text())
                .then(data => {
                    totalDataInput.value = `Total data untuk user yang dipilih: ${data}`;
                });
        } else {
